...
The distribution most often used in ML (and statistics) is the Gaussian distribution, also called the _normal distribution_. The reason this distribution is so commonly used is because it is empirically observed many times and, perhaps more importantly, it is mathematically simple to use. This post will delve into the definition and properties of the Gaussian distribution<d-footnote>See <a href="https://www.microsoft.com/en-us/research/uploads/prod/2006/01/Bishop-Pattern-Recognition-and-Machine-Learning-2006.pdf">Bishop 2.3</a> for a much more extensive introduction to the Gaussian distribution.</d-footnote>.

<br>

# Definition

A random variable $x$ is said to have a Gaussian distribution if its PDF has the following form:
...
The conditioning sign (or semi-colon) in $\mathcal{N}\left(x\,\mid \,\mu,\sigma^{2}\right)$ shows that $x$ is the variable of interest, while $\mu$ and $\sigma$ are the parameters defining the distribution (so, given a $\mu$ and a $\sigma$, we know the PDF of $x$). 
...
The multivariate version for an $d$-dimensional random vector $x\in\mathbb{R}^{d}$
is defined as:
...
and in this case $\mu$ is also a vector, and $\Sigma$ is a symmetrical $n\times n$ matrix. The term $D_{M}\left(x\,\mid \,\mu,\Sigma\right)^{2}=\left(x-\mu\right)^{T}\Sigma^{-1}\left(x-\mu\right)$ is often called the _Mahalanobis distance_ and is denoted with $\Delta$. The multivariate version of the Gaussian distribution is also called the _multivariate normal_ (MVN) distribution.
...
In 1D, the Gaussian distribution takes the form of the famous bell curve in figure 1 and is easy to view. However, in multiple dimensions it is not so clear what the geometry of the distribution looks like. We can gain insight by considering the EVD of the covariance matrix $\Sigma$ (remember, this decomposition exists since $\Sigma$ is symmetric):

$$
\Sigma=UDU^{T}
$$
where $D$ is a diagonal matrix with the eigenvalues $\lambda_{i}$ on the diagonal and $U$ is an orthonormal matrix (so $UU^{T}=I$) with the eigenvectors $u_{i}$ as its rows, such that for all $i$:
...
\end{equation}
$$
Recall that eigenvectors are orthogonal to each other, and we can choose normalized eigenvectors, so for all $i\neq j$ we have $u_{i}^{T}u_{j}=0$ and $u_{i}^{T}u_{i}=1$. 
...
A common approach in many probabilistic modeling methods is to ignore the normalization constant of a distribution, either because it is hard to calculate or because writing it down completely is a chore. Moreover, it is sometimes easier to look at distributions in log space. So, for instance, the Gaussian distribution will be written as follows:
$$
\begin{align}
...
\end{align}
$$
where the constant term is only constant with respect to $x$ (not necessarily $\Sigma$ and $\mu$). If we expand the quadratic expression further, we have:
$$
\begin{equation}
...
is a quadratic function</d-footnote> is a Gaussian distribution. More explicitly, whenever we have a distribution of the form:
$$
\begin{equation}
p\left(x\right)\propto\exp\left[-x^{T}\Gamma x+b^{T}x+c\right]\label{eq:log-quad}
\end{equation}
$$
then we know that it will be Gaussian. This is useful knowledge, as Gaussian distributions have nice attributes that can be exploited. Note, however, that in the above example $\Gamma$ and $b$ are _not_ the mean and (inverse) covariance. We will typically need to do a bit more algebra to find the mean and covariance of the distribution, which will allow us to write the Gaussian in "standard form" - as $\mathcal{N}\left(x\ |\ \mu,\Sigma\right)$.

<br>

## Derivatives of the Log-PDF

As a by-product of the above, the derivatives of the log-PDF of the Gaussian are surprisingly informative and are given by:
$$
\begin{align}
...
\end{align}
$$
This already tells us a lot. For instance, if we want to find the maximum, then we can ensure that the above equals 0 and we get:
...
An important property of the multivariate Gaussian distribution is that if two sets of variables are jointly Gaussian, then the conditional distribution of one set given the other is also Gaussian.

Consider a Gaussian variable separated into two parts:
...
Since the covariance is symmetrical, we know that $\Sigma_{ab}=\Sigma_{ba}^{T}$, $\Sigma_{aa}$, and $\Sigma_{bb}$ are symmetrical. Actually, it will be easier to use the precision matrix $\Lambda=\Sigma^{-1}$ and divide it in the same manner:
...
Let's start by finding an expression for the conditional distribution $p\left(x_{a}\mid x_{b}\right)$. We can find this distribution by evaluating the distribution of $p\left(x_{a},x_{b}\right)$ while fixing $x_{b}$ to a certain value and re-normalizing (the conditional distribution is a legal distribution). We will start by rewriting the quadratic term and its parts:
...
This is still a quadratic expression w.r.t. $x_{a}$, so the conditional distribution $p\left(x_{a}\mid x_{b}\right)$ will also be Gaussian. Because the form of the Gaussian is not very flexible, as long as we find what the quadratic term equals (the one in the exponent), the normalization will work itself out since the conditional is also a distribution that must integrate to 1. 
...
So we know that the covariance is equal to $\Sigma_{a\mid b}=\Lambda_{aa}^{-1}$. Using this newfound knowledge, we can find the mean if we can rewrite equation \eqref{eq:mean-form-1} as $\Lambda_{aa}\left(x_{a}-\mu_{a\mid b}\right)$ for some $\mu_{a\mid b}$. Let's try to do this. Recall that $\Lambda_{aa}$ is invertible, so we can write:
...
Note that in this case, the conditional distribution is much easier to describe in terms of the precision matrix than the covariance matrix.
...
When implementing the code for this, it may be simpler to save the precision matrix and the covariance matrix in memory to easily compute the conditional distribution.
...
Another important property of the Gaussian distribution is that its marginals are also Gaussian, which we will show here.

Again, we consider a Gaussian variable separated into two parts:
...
We will again define $\Lambda=\Sigma^{-1}$. We want to find $p\left(x_{a}\right)$.

{%details Click here to see the derivation%}

Our battle plan is to first find all dependencies on $x_{b}$ and integrate them out. If we can do this without losing track of $x_{a}$, we will be able to find the marginal distribution and win. Notice that the Mahalanobis distance is quadratic in $x_{b}$ (as usual), so the final form we will reach will be something like:

$$
p\left(x_{a}\right)=f\left(x_{a}\right)\intop\mathcal{N}\left(x_{b}\,\mid \,\mu_{x_{b}},\Sigma_{x_{b}}\right)dx_{b}=f\left(x_{a}\right)
$$

So, let’s try to expand the Mahalanobis distance (the part in the
...
which is definitely Gaussian! We were allowed to do the second move because the integral will be the normalization term of the Gaussian, which is a function of $\Lambda_{b}$—constant with respect to $y_{a}$ (and is also eaten up by the $\propto$ sign).
...
and all that remains is to find what the covariance is in terms of $\Sigma$. To do this, we use the same identity we saw in the previous example:
...
which really makes you wonder why we did all that hard work.
...
We saw the so-called “derivative trick” and how completing the square can also be helpful, but it might not be obvious when to use each approach. First, remember that whenever we see a distribution of the form:
...
then $p\left(x\right)$ and $p\left(x\mid y\right)$ will be Gaussians<d-footnote>This is a slightly more general statement than what we showed here, but you can verify the validity for yourself in these cases as well.</d-footnote>, and we usually want to find the "Gaussian form" we know and love. Once we figure that out, we can try to ask "how can we find the Gaussian form?", and the answer will usually be one of the following methods:
...
Once you fully understand why each method works, it will become clear when you should use each of them.
...
Now that we know what the Gaussian distribution is and its important properties, we can start using it for some real statistical problems. The next post will be dedicated to estimating $\mu$ and $\Sigma$ from observed data, after which we will move on to utilizing the Gaussian distribution in regression.
<br>
