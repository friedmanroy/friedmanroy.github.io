...
description: A high-level description of the frequentist and Bayesian approaches, their differences and some shared qualities.
date: 2022-10-28
authors:
  - name: Roy Friedman
    affiliations:
...
Many tasks in statistics and machine learning can be summarized as attempting to extract information from data. Usually, we assume that we are given a set of data points:
$$
\begin{equation}
...
The driving force of many analyses will be to attempt to find the distribution from which the data points in $\mathcal{D}$ were initially drawn. This task is called _modeling_, and (broadly speaking) there are two methodologies for doing so—the frequentist approach and the Bayesian approach.

These two philosophies are often framed as contradictory, but they don't necessarily have to compete with each other. The frequentist problem can often be framed using the Bayesian philosophy, and vice versa. Understanding both sides of the argument can help build a stronger foundation and intuition for machine learning in general.

In this post, I will attempt to highlight the differences between the approaches, describe the limitations of each, and perhaps why they are philosophically at odds with one another.


<br>
# Frequentist Approach

The frequentist outlook can be further split into two categories: classical and probabilistic. The first is closer to what is regularly taught in intro2ML type classes, while the second is somewhat more structured. The core characteristics of the frequentist philosophy (both probabilistic and not) can be summarized as:
...
---
All of these definitions can be pretty confusing, but hopefully, a simple example will clarify their meanings. 

#### Example: Coin Toss

Suppose we observe the outcomes of coin tosses, where each data point $x$ is either "heads" (denoted by $H$ ), or "tails" (denoted by $T$ ). Such a dataset might look something like:
...
How should we go about doing this? The frequentist approach suggests defining some loss function $L(\mathcal{D};\ \theta)$ which will act as a criterion for the quality of our choice of $\theta$ , which we will then attempt to minimize it.
...
In the most general form, frequentist machine learning requires two objects, as mentioned before: a set of parameters $\theta$ that define a hypothesis class and a loss function $L\left(\mathcal{D};\,\theta\right)$. Typically, the loss function is chosen so that it is minimized by the desired outcome, although this can be hard to control in many real-life applications. 

Framing the problem of machine learning in this manner results in a deterministic algorithm, whose correctness is then left to prove. Examples of such algorithms include decision trees, support vector machines (SVMs), and _k_-means; their definitions and solutions are inherently not probabilistic. 

---

#### Example: Coin Toss (again)
Through a simple thought experiment, we can convince ourselves that for any value of $\theta$, if we look at a dataset of size $N$ and count the number of heads $N_H$ in that dataset, then at the limit of very large $N$ we will usually observe:
...
The value of $\theta$ that minimizes this loss is, rather intuitively, $\hat{\theta}=\frac{N_H}{N}$, and we will say that this is our guess for the value of $\theta$ (which is why I added the $\hat{}$ to remind us that this isn’t the true value, only our guess). This estimate has the nice property of being correct in the limit of infinite data if we believe that this is truly the process that generated the data.

$$
\hat{\theta} \stackrel{N\to\infty}{=}\theta
$$

This example is simplified to the point of being cartoonish but serves as a very basic sketch of one method for estimation.
...
One of the most common probabilistic criteria used to estimate parameters is called the likelihood. Given a data set $\mathcal{D}$, we define the likelihood as:
$$
...
Having defined this criterion, the next natural step is to maximize the likelihood to estimate the parameters $\theta$.
...
\end{equation}
$$
The result is the same (since the logarithm is a strictly monotonically increasing function), but this involves maximizing a sum instead of a product, which is usually easier.
...
The Bayesian philosophy assumes that we have some knowledge about the distribution from which the points were drawn ahead of time; i.e., we assume that the parameters themselves have some distribution $p\left(\theta\right)$ . This distribution is usually called the _prior distribution_, because it reflects our prior knowledge. This means that there is no single true value for $\theta$ , rather that a distribution of $\theta$s could have given rise to the data. That is, unlike the frequentist view where $\theta$ _is by definition not probabilistic, under the Bayesian view we assume that there is some distribution over $\theta$ s.

In this new outlook, instead of trying to find the $\theta$ that generated the data, we will try to update our knowledge regarding which values $\theta$ could have had to create the data. We will want to find the _posterior distribution_ $p\left(\theta\mid\mathcal{D}\right)$, so called because we update our beliefs _after_ the fact (in Latin "post" means "after," while "prior" means "before"). Using Bayes' law, we can describe this using the prior and likelihood distributions:
$$
\begin{equation}
...
Finally, many times we are not interested in the posterior over parameter values $\theta$, but actually only care about the predictions. In this case, we can define the _posterior predictive distribution_ (PPD) as:
$$
\begin{equation}
p\left(d^{*}\mid\ \mathcal{D}\right)\stackrel{\Delta}{=}\int p\left(d^{*}\mid\ \theta\right)p\left(\theta\mid\,\mathcal{D}\right)d\theta
\end{equation}
$$
...
The only real difference in this example from the MLE version is that now we want to weight each outcome by the prior distribution. The prior distribution conveys our beliefs about the outcomes, which is why it is sometimes called a *subjective* probability.

For this example, having heard about this experiment and seeing it conducted many times in the past, my belief is that it is more likely that the coin is a fair coin, i.e. $\theta\approx 1/2$ than otherwise. However, I additionally feel that it is not impossible for the coin to be unfair (in either direction), so the prior chosen according to previous experiments I've seen in the past is:
...
Similarly to probabilistic ML, Bayesian ML allows us to elegantly define our problems and how to solve them. All we need to do is define the likelihood function and our prior, and we have the solution—find the posterior distribution. This contrasts with the classical ML approach, where a loss function and algorithm must be found such that (a) minimizing the loss gives a solution to the problem and (b) the algorithm actually minimizes the loss. 

Additionally, the Bayesian approach allows us to inject prior knowledge of the problem in order to generate a solution through the density $p\left(\theta\right)$ . This prior knowledge can be very helpful if not much data has been observed. Finally, under certain circumstances, it can be proved that the Bayesian approach will give optimal solutions.

## Problems with the Bayesian Approach

The biggest flaw (and strength!) of Bayesian ML is the prior distribution $p\left(\theta\right)$ . Since the prior can _heavily_ affect the solution, it is imperative that it is chosen to correctly reflect the space of solutions. But often, we have no prior knowledge about the solutions; for instance, how can we know what the distribution over the parameters of a neural network should look like? This is an ambiguous decision that has to be made, and often it is not obvious how to make it. In practice, this means the prior is often arbitrarily chosen and does not necessarily reflect the distribution over parameters we want. 

An additional difficulty of Bayesian methods is calculating the posterior $p\left(\theta\mid\,\mathcal{D}) \). While the learning "algorithm" in Bayesian ML is conceptually straightforward, the posterior is often intractable. In such cases, approximations or point estimates must be used. These approximations are often based on MCMC samples from the posterior or variational methods, whose quality can be difficult to verify.

However, both of these flaws are usually shared by methods in classic ML. Typically, the loss used in such methods is heuristically chosen - much like the prior in Bayesian approaches. Furthermore, while it can be shown that the solution is reached when the loss is minimized, it is usually difficult (if not impossible) to show that the algorithm has actually reached a minimum.
...
Actually, we did something extremely fishy when we said that $p\left(\theta\right)$ is "uniform" — this isn't possible in many cases! A uniform probability over all of the real line $\mathbb{R}$ is impossible... so how can we even talk about this so-called "uninformative prior"? While this is true, as long as the posterior $p\left(\theta \mid \mathcal{D}\right)$ is well-defined, the MAP and MMSE estimates will still exist. In this special case, the frequentist and Bayesian worlds collide, and it will be useful to keep this fact in mind.

<br>

# Discussion

Both frequentist and Bayesian philosophies offer useful methods but also have limitations. Therefore, this series of posts will generally examine both approaches in tandem. The Bayesian approach often complements the frequentist one by providing a way to incorporate existing knowledge when making predictions.

Until now, the discussion has remained general and lacked specifics. Future posts will provide more detail, focusing on situations where estimators derived through Bayesian methods are usually best. To determine this, we must first define what constitutes an “optimal” estimator, a process that relies on principles of decision theory.
