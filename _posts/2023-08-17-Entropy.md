...
title: The Entropy of Waiting at a Bus Stop 

description: Busses don't always arrive on time. Finding a distribution to model waiting times for these traunt busses sounds easy, but what do we do if we find out that mutiple models seem to describe the situation perfectly? Choosing among numerous distributions under a set of constraints, such as the average waiting time for a bus, is an integral part of scientific discovery. In this post, we derive one of the most well known principles for doing so, max-entropy methods. Using this principle, generic distributions under a set of constraints take the form of Boltzmann distributions, thoroughly-explored in statistical mechanics.
...
The same principles originally introduced for statistical physics are now used widely in machine learning and statistics. Since the described method is simply a way to find distributions that are generic while retaining known information, it is a recurring principle used for scientific discovery and modeling in a range of fields.
